"""This module contains the constant address values used when reading in game memory"""


class RAC3STATUS:
    """Constant addresses for in game memory values"""
    APCODE = 51000000
    RATCHET_MOBY_POINTER = 0x001A71A0
    ACTION = 0x001A71A4
    PREV_ACTION = 0x001A71B0
    SECOND_PREV_ACTION = 0x001A71BC
    ACTION_TYPE = 0x001A71AC
    LEVEL_TABLE = 0x001425C0
    WRENCH_LEVEL= 0x001425C9
    BOLTS = 0x00142660
    MAX_HEALTH = 0x00142668
    LAST_USED_0 = 0x00142670
    LAST_USED_1 = 0x00142674
    LAST_USED_2 = 0x00142678
    LAST_USED_3 = 0x0014267C
    LAST_USED_4 = 0x00142680
    LAST_USED_5 = 0x00142684
    WRENCH_EQUIPPED = 0x00142690
    CHALLENGE_MODE = 0x00142692
    NANOTECH_EXP = 0x00142694
    ARMOR = 0x001426A0
    HELMET = 0x001426D8
    CRYSTALS_CURRENT = 0x001426A2
    CRYSTALS_TRADED = 0x0014275E
    MULTIPLIER = 0x001426BA
    ROBONOIDS = 0x0014275C
    ITEM_AMMO_ADDRESS = 0x001427F0
    ITEM_UNLOCK_ADDRESS = 0x00142CA0
    ITEM_UNLOCK_ADDRESS_2_OFFSET = 0xA0
    ITEM_XP_ADDRESS = 0x00142DE0
    PLANET_SLOT_ADDRESS = 0x00143050
    MAIN_MENU = 0x0016C598
    MAP_CHECK = 0x0016C5A0
    GAME_LAUNCHED = 0x0011F3A0
    JACKPOT_TIMER = 0x001A4E10
    INFERNO_TIMER = 0x001A4E14
    QWARK_AMMO = 0x001A66E8
    PLAYER_TYPE = 0x001A71C4
    HEALTH = 0x001A7430
    NANOPAK_HEALTH = 0x001427B3
    GIANT_CLANK_HEALTH = 0x001A6664
    JACKPOT = 0x001A74A8
    WEAPON_LOCK = 0x001A74A9
    PAUSE_STATE = 0x001D668C
    PLANET_LOAD = 0x001D6674
    PLAYER_ACTIONABLE = 0x001CD098
    EQUIPPED = 0x001D4C40
    QUICK_SELECT = 0x001D4C60
    PLANET = 0x001D545C
    SHIP_CONFIG = 0x001D54D0
    SHIP_SKIN = 0x001D54D2
    PLAYER_SKIN = 0x001427AB
    PLAYER_SKIN_2 = 0x001C5D67
    ALLOW_SHIP = 0x001D5533
    DEATH_COUNT = 0x001D4CA0
    VIDCOMIC = 0x001D554F
    RATCHET_X = 0x001A4C60
    RATCHET_Y = 0x001A4C64
    RATCHET_Z = 0x001A4C68
    RATCHET_ANGLE = 0x001A4C78
    FORCE_RELOAD = 0x001A71D2
    NO_CLANK = 0x001A7464
    PACK_EQUIP = 0x001A7200
    ENTRANCE_X = 0x0016C5F0
    ENTRANCE_Y = 0x0016C5F4
    ENTRANCE_Z = 0x0016C5F8
    ENTRANCE_ANGLE = 0x0016C608
    WRITE_INPUT_1 = 0x001CD0C2
    WRITE_INPUT_2 = 0x001CD142
    READ_INPUT = 0x001CD260
    VEHICLE_POINTER = 0x001A65C0
    HELD_ITEM = 0x001A5E08
    MIRROR_UNIVERSE = 0x001D5477
    BLACK_SCREEN = 0x00142360
    BLACK_SCREEN_2 = 0x00142150
    INFOBOT_BASE = 0x001D5530
    VISITED_BASE = 0x001D5570
    HIDE_WEAPON = 0x001A71D0
    HIDEOUT_MOBY_TABLE_START = 0x01D26640
    PDA_VENDOR_MOBY_ID = 0x0122
    POS_X = 0x001A4C60
    POS_Y = 0x001A4C64
    POS_Z = 0x001A4C68
    INVISIBLE = 0x001A71C5
    DISARM = 0x001A71CD
    WRENCH_ONLY = 0x001A71C8  # Used when Ratchet is in water, but only allows wrench on land.
    IN_VEHICLE = 0x001A71CF
    HOLOSTAR_CLANK_FIX = 0x00142713  # Same address as Holostar: Return to your ship, needed for Clank fix
    ESCAPED_LEVIATHAN = 0x00142700
    ZELDRIN_START_LEVIATHAN = 0x001BBCEA
    ZELDRIN_END_LEVIATHAN = 0x001BBCEB
    LIGHTSABER_UNLOCK = 0x001427A1
    WRENCH_REPLACEMENT_CHEAT = 0x001D5479
    SOFTLOCK_TIMER = 0x001A4DE2
    RANGER_COMPLETION_TABLE_START = 0x00142738
    WEAPON_DEMO = 0x001427B6
    SHORT_PAUSE = 0x001A724F
    LONG_PAUSE = 0x001A72CF
    SEEN_NGPLUS_POPUP = 0x001427C4
    OMEGA_ARSENAL_TROPHY = 0x0014279E
    HELP_DESK_VOICE = 0x00143954
    HELP_DESK_TEXT = 0x00143955
    SEWERS_VISITED = 0x001D558C
    BOLT_PACKS = 0x00142DE4
    NANOTECH_EXP_PACKS = 0x00142DE8
    WEAPON_LEVEL_PACKS = 0x00142DEC
    JACKPOT_PACKS = 0x00142DF0
    # these values are high enough to vary based on planet
    VIDCOMIC_PAUSE = 0x001D7C48
    PLANET_LOAD_BASE = 0x001DE544
    PLANET_STRING_TABLE_BASE = 0x001DE6C0
    PLANET_BOLT_DIFFERENCE_BASE = 0x001DEDC0
    MARCADIA_SECTION = 0x001DEF34
    PAUSE_BASE = 0x001DF068
    RESPAWN_BASE = 0x002348D0
    VENDOR_BASE = 0x002418C0
    WRENCH_FUNCTION_BASE_NTSC = 0x004DFB38
    WRENCH_FUNCTION_BASE_PAL = 0x004E3F68
    MAGNETISM_FUNCTION_BASE_NTSC = 0x004B98CC
    GRAV_BOOTS_EVERYWHERE_BASE_NTSC = 0x004B9808
    CHECK_FLOOR_BASE_NTSC = 0x004B97FC
